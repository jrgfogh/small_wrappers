...
concept lazy_init_specialization =
    ...
    std::is_same_v<decltype(*std::declval<T const>()),
...
static_assert(lazy_init_specialization<sw::lazy_init<
                  not_default_constructible, not_default_constructible (*)()>>);
static_assert(
    lazy_init_specialization<sw::lazy_init<std::string, std::string (*)()>>);
static_assert(lazy_init_specialization<
              sw::lazy_init<std::shared_ptr<int>, std::nullptr_t (*)()>>);
...
TEST(LazyInit, DoesNotRequireAssignableData) {
  sw::lazy_init<not_assignable> li{[] { return not_assignable{}; }};
  *li;
}

TEST(LazyInit, StarOperator) {
  for (auto i : std::ranges::iota_view(5, 10)) {
    sw::lazy_init<int> li{[&] { return i; }};
...
TEST(LazyInit, ArrowOperator) {
  for (auto i : std::ranges::iota_view(5, 10)) {
    sw::lazy_init<comparable> li{[&] { return comparable{i}; }};
...
TEST(LazyInit, CompareEquality) {
  sw::lazy_init<comparable> li0{[] { return comparable{0}; }};

  sw::lazy_init<comparable> li1{[] { return comparable{1}; }};
...
// This test ensures that we don't accidentally require the value type to be
// equality comparable, unless we actually try to compare two values.
TEST(LazyInit, UncomparableClassCanBeConstructed) {
  sw::lazy_init<not_comparable> li{[] { return not_comparable{0}; }};
...
//	EXPECT_THROW((sw::lazy_init<int, std::function<int()>>{init}),
//std::exception);
// }
